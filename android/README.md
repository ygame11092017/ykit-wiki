...
Replace `{A}` string with `App_Scheme` in file `config.xml`
...
- Add the following to `Application` tag
  ...
  <provider
      ...
      android:authorities="com.facebook.app.FacebookContentProvider{B}"
...
Replace `{B}` string with `FacebookAppID` in file `config.xml`

Replace `{C}` string with `Ads_Id` in file `config.xml`
